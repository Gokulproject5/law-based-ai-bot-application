...
export default function EmergencyButton({ compact = false }) {
        ...
        return (
            <div className="relative">
                <button
                    ...
                    className={`p-2 rounded-full transition-all duration-300 flex items-center justify-center ${isOpen ? 'bg-gray-800' : 'bg-red-600 hover:bg-red-700'}`}
                >
                    {isOpen ? <X className="text-white" size={16} /> : <AlertCircle className="text-white" size={20} />}
                </button>

                {isOpen && (
                    <div className="absolute top-full right-0 mt-2 bg-white rounded-2xl shadow-2xl p-4 border border-red-100 flex flex-col gap-3 animate-in slide-in-from-top-2 fade-in duration-200 w-64 z-[100]">
                        <a href="tel:112" className="flex items-center gap-3 bg-red-600 text-white px-4 py-3 rounded-xl font-bold hover:bg-red-700 transition shadow-lg w-full">
                            <AlertCircle size={20} />
                            <span className="text-xs">{t('call_all')}</span>
                        </a>
                        <a href="tel:181" className="flex items-center gap-3 bg-pink-600 text-white px-4 py-3 rounded-xl font-bold hover:bg-pink-700 transition shadow-lg w-full">
                            <Phone size={20} />
                            <span className="text-xs">{t('call_women')}</span>
                        </a>
                        <a href="tel:1930" className="flex items-center gap-3 bg-blue-600 text-white px-4 py-3 rounded-xl font-bold hover:bg-blue-700 transition shadow-lg w-full">
                            <Shield size={20} />
                            <span className="text-xs">{t('call_cyber')}</span>
                        </a>
                    </div>
                )}
            </div>
    ...
    return (
        <div className="hidden md:flex fixed bottom-40 right-6 z-40 flex-col items-end gap-2 pointer-events-none">
            {isOpen && (
